...
import { bundles, products } from "@/lib/site-data";
    ...
    <section className="relative py-24 lg:py-32">
      <div className="max-w-[1320px] mx-auto px-6 lg:px-12">
        <div className="max-w-2xl">
          <span className="font-mono text-xs tracking-widest text-primary uppercase">{t("bundles.eyebrow")}</span>
          <h2 className="mt-4 font-display text-4xl md:text-5xl lg:text-6xl tracking-tight text-balance">
            {t("bundles.heading")}
          </h2>
          <p className="mt-5 text-lg text-muted-foreground leading-relaxed">{t("bundles.subheading")}</p>
        </div>
          ...
          {bundles.map((bundle, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="font-display text-2xl tracking-tight">{t(`bundles.items.${bundle.key}.name`)}</h3>
                <span className="rounded-full bg-primary/15 px-2.5 py-1 text-xs font-medium text-foreground">
                  {t(`bundles.items.${bundle.key}.save`)}
                </span>
              </div>
              <p className="mt-2 text-muted-foreground leading-relaxed">
                {t(`bundles.items.${bundle.key}.description`)}
              </p>

              <ul className="mt-6 space-y-3">
                {bundle.productKeys.map((slug) => (
